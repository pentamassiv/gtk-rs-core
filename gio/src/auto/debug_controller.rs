...
pub trait DebugControllerExt: IsA<DebugController> + sealed::Sealed + 'static {
    ...
    fn is_debug_enabled(&self) -> bool {
        unsafe {
            from_glib(ffi::g_debug_controller_get_debug_enabled(self.as_ref().to_glib_none().0))
        }
    }
    ...
    fn set_debug_enabled(&self, debug_enabled: bool) {
        unsafe {
            ffi::g_debug_controller_set_debug_enabled(self.as_ref().to_glib_none().0, debug_enabled.into_glib());
        }
    }
    ...
    fn connect_debug_enabled_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_debug_enabled_trampoline<P: IsA<DebugController>, F: Fn(&P) + 'static>(this: *mut ffi::GDebugController, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(DebugController::from_glib_borrow(this).unsafe_cast_ref())
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"notify::debug-enabled\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(notify_debug_enabled_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
