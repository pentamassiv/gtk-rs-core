...
impl UnixFDMessage {
    ...
    pub fn new() -> UnixFDMessage {
        unsafe {
            SocketControlMessage::from_glib_full(ffi::g_unix_fd_message_new()).unsafe_cast()
        }
    }
    ...
    pub fn with_fd_list(fd_list: &impl IsA<UnixFDList>) -> UnixFDMessage {
        unsafe {
            SocketControlMessage::from_glib_full(ffi::g_unix_fd_message_new_with_fd_list(fd_list.as_ref().to_glib_none().0)).unsafe_cast()
        }
    }
}
...
pub trait UnixFDMessageExt: IsA<UnixFDMessage> + sealed::Sealed + 'static {
    ...
    fn fd_list(&self) -> UnixFDList {
        unsafe {
            from_glib_none(ffi::g_unix_fd_message_get_fd_list(self.as_ref().to_glib_none().0))
        }
    }
}
