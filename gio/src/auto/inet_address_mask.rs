...
pub trait InetAddressMaskExt: IsA<InetAddressMask> + sealed::Sealed + 'static {
    ...
    fn connect_address_notify<F: Fn(&Self) + Send + Sync + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_address_trampoline<P: IsA<InetAddressMask>, F: Fn(&P) + Send + Sync + 'static>(this: *mut ffi::GInetAddressMask, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"notify::address\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(notify_address_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "family")]
    fn connect_family_notify<F: Fn(&Self) + Send + Sync + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_family_trampoline<P: IsA<InetAddressMask>, F: Fn(&P) + Send + Sync + 'static>(this: *mut ffi::GInetAddressMask, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(InetAddressMask::from_glib_borrow(this).unsafe_cast_ref())
        }
        unsafe {
    ...
    fn connect_length_notify<F: Fn(&Self) + Send + Sync + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_length_trampoline<P: IsA<InetAddressMask>, F: Fn(&P) + Send + Sync + 'static>(this: *mut ffi::GInetAddressMask, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(InetAddressMask::from_glib_borrow(this).unsafe_cast_ref())
        }
        unsafe {
