...
pub trait AppLaunchContextExt: IsA<AppLaunchContext> + sealed::Sealed + 'static {
    ...
    fn connect_launch_failed<F: Fn(&Self, &str) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn launch_failed_trampoline<P: IsA<AppLaunchContext>, F: Fn(&P, &str) + 'static>(this: *mut ffi::GAppLaunchContext, startup_notify_id: *mut libc::c_char, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(AppLaunchContext::from_glib_borrow(this).unsafe_cast_ref(), &glib::GString::from_glib_borrow(startup_notify_id))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"launch-failed\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(launch_failed_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
    ...
    fn connect_launch_started<F: Fn(&Self, &AppInfo, Option<&glib::Variant>) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn launch_started_trampoline<P: IsA<AppLaunchContext>, F: Fn(&P, &AppInfo, Option<&glib::Variant>) + 'static>(this: *mut ffi::GAppLaunchContext, info: *mut ffi::GAppInfo, platform_data: *mut glib::ffi::GVariant, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(AppLaunchContext::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(info), Option::<glib::Variant>::from_glib_borrow(platform_data).as_ref().as_ref())
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"launch-started\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(launch_started_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "launched")]
    fn connect_launched<F: Fn(&Self, &AppInfo, &glib::Variant) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn launched_trampoline<P: IsA<AppLaunchContext>, F: Fn(&P, &AppInfo, &glib::Variant) + 'static>(this: *mut ffi::GAppLaunchContext, info: *mut ffi::GAppInfo, platform_data: *mut glib::ffi::GVariant, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(AppLaunchContext::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(info), &from_glib_borrow(platform_data))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"launched\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(launched_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
