...
impl ProxyResolver {
    ...
    #[allow(clippy::should_implement_trait)]    pub fn default() -> ProxyResolver {
        unsafe {
...
pub trait ProxyResolverExt: IsA<ProxyResolver> + sealed::Sealed + 'static {
    ...
    fn lookup(&self, uri: &str, cancellable: Option<&impl IsA<Cancellable>>) -> Result<Vec<glib::GString>, glib::Error> {
        unsafe {
    ...
    fn lookup_async<P: FnOnce(Result<Vec<glib::GString>, glib::Error>) + 'static>(&self, uri: &str, cancellable: Option<&impl IsA<Cancellable>>, callback: P) {
        ...
        unsafe extern "C" fn lookup_async_trampoline<P: FnOnce(Result<Vec<glib::GString>, glib::Error>) + 'static>(_source_object: *mut glib::gobject_ffi::GObject, res: *mut crate::ffi::GAsyncResult, user_data: glib::ffi::gpointer) {
            let mut error = ptr::null_mut();
        ...
        unsafe {
            ffi::g_proxy_resolver_lookup_async(self.as_ref().to_glib_none().0, uri.to_glib_none().0, cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box_::into_raw(user_data) as *mut _);
        }
    }

    
    fn lookup_future(&self, uri: &str) -> Pin<Box_<dyn std::future::Future<Output = Result<Vec<glib::GString>, glib::Error>> + 'static>> {
