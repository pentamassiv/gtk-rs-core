...
impl DebugControllerDBus {
    ...
    pub fn new(connection: &DBusConnection, cancellable: Option<&impl IsA<Cancellable>>) -> Result<Option<DebugControllerDBus>, glib::Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let ret = ffi::g_debug_controller_dbus_new(connection.to_glib_none().0, cancellable.map(|p| p.as_ref()).to_glib_none().0, &mut error);
...
pub trait DebugControllerDBusExt: IsA<DebugControllerDBus> + sealed::Sealed + 'static {
    ...
    fn connect_authorize<F: Fn(&Self, &DBusMethodInvocation) -> bool + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn authorize_trampoline<P: IsA<DebugControllerDBus>, F: Fn(&P, &DBusMethodInvocation) -> bool + 'static>(this: *mut ffi::GDebugControllerDBus, invocation: *mut ffi::GDBusMethodInvocation, f: glib::ffi::gpointer) -> glib::ffi::gboolean {
            let f: &F = &*(f as *const F);
            f(DebugControllerDBus::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(invocation)).into_glib()
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"authorize\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(authorize_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
