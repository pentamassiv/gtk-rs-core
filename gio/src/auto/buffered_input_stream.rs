...
pub struct BufferedInputStreamBuilder {
        ...
        fn new() -> Self {
            Self { builder: glib::object::Object::builder() }
                            ...
                            Self { builder: self.builder.property("buffer-size", buffer_size), }
                        }
...
pub trait BufferedInputStreamExt: IsA<BufferedInputStream> + sealed::Sealed + 'static {
    ...
    fn fill(&self, count: isize, cancellable: Option<&impl IsA<Cancellable>>) -> Result<isize, glib::Error> {
        unsafe {
    ...
    fn fill_async<P: FnOnce(Result<isize, glib::Error>) + 'static>(&self, count: isize, io_priority: glib::Priority, cancellable: Option<&impl IsA<Cancellable>>, callback: P) {
        ...
        unsafe extern "C" fn fill_async_trampoline<P: FnOnce(Result<isize, glib::Error>) + 'static>(_source_object: *mut glib::gobject_ffi::GObject, res: *mut crate::ffi::GAsyncResult, user_data: glib::ffi::gpointer) {
            let mut error = ptr::null_mut();
        ...
        unsafe {
            ffi::g_buffered_input_stream_fill_async(self.as_ref().to_glib_none().0, count, io_priority.into_glib(), cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box_::into_raw(user_data) as *mut _);
        }
    }

    
    fn fill_future(&self, count: isize, io_priority: glib::Priority) -> Pin<Box_<dyn std::future::Future<Output = Result<isize, glib::Error>> + 'static>> {
    ...
    fn peek_buffer(&self) -> Vec<u8> {
        unsafe {
            ...
            let ret = FromGlibContainer::from_glib_none_num(ffi::g_buffered_input_stream_peek_buffer(self.as_ref().to_glib_none().0, count.as_mut_ptr()), count.assume_init() as _);
            ret
        }
    }
    ...
    fn connect_buffer_size_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_buffer_size_trampoline<P: IsA<BufferedInputStream>, F: Fn(&P) + 'static>(this: *mut ffi::GBufferedInputStream, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
