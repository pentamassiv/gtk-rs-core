...
pub trait FileMonitorExt: IsA<FileMonitor> + sealed::Sealed + 'static {
    ...
    fn emit_event(&self, child: &impl IsA<File>, other_file: &impl IsA<File>, event_type: FileMonitorEvent) {
        unsafe {
            ffi::g_file_monitor_emit_event(self.as_ref().to_glib_none().0, child.as_ref().to_glib_none().0, other_file.as_ref().to_glib_none().0, event_type.into_glib());
        }
    }
    ...
    fn connect_changed<F: Fn(&Self, &File, Option<&File>, FileMonitorEvent) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn changed_trampoline<P: IsA<FileMonitor>, F: Fn(&P, &File, Option<&File>, FileMonitorEvent) + 'static>(this: *mut ffi::GFileMonitor, file: *mut ffi::GFile, other_file: *mut ffi::GFile, event_type: ffi::GFileMonitorEvent, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(FileMonitor::from_glib_borrow(this).unsafe_cast_ref(), &from_glib_borrow(file), Option::<File>::from_glib_borrow(other_file).as_ref().as_ref(), from_glib(event_type))
        }
        unsafe {
    ...
    fn connect_cancelled_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_cancelled_trampoline<P: IsA<FileMonitor>, F: Fn(&P) + 'static>(this: *mut ffi::GFileMonitor, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"notify::cancelled\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(notify_cancelled_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "rate-limit")]
    fn connect_rate_limit_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_rate_limit_trampoline<P: IsA<FileMonitor>, F: Fn(&P) + 'static>(this: *mut ffi::GFileMonitor, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(FileMonitor::from_glib_borrow(this).unsafe_cast_ref())
        }
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"notify::rate-limit\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(notify_rate_limit_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
