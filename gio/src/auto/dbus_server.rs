...
impl DBusServer {
    ...
    pub fn new_sync(address: &str, flags: DBusServerFlags, guid: &str, observer: Option<&DBusAuthObserver>, cancellable: Option<&impl IsA<Cancellable>>) -> Result<DBusServer, glib::Error> {
        unsafe {
            let mut error = ptr::null_mut();
            let ret = ffi::g_dbus_server_new_sync(address.to_glib_none().0, flags.into_glib(), guid.to_glib_none().0, observer.to_glib_none().0, cancellable.map(|p| p.as_ref()).to_glib_none().0, &mut error);
    ...
    pub fn connect_new_connection<F: Fn(&Self, &DBusConnection) -> bool + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn new_connection_trampoline<F: Fn(&DBusServer, &DBusConnection) -> bool + 'static>(this: *mut ffi::GDBusServer, connection: *mut ffi::GDBusConnection, f: glib::ffi::gpointer) -> glib::ffi::gboolean {
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, b"new-connection\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(new_connection_trampoline::<F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "active")]
    pub fn connect_active_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_active_trampoline<F: Fn(&DBusServer) + 'static>(this: *mut ffi::GDBusServer, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
    ...
    pub fn connect_client_address_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_client_address_trampoline<F: Fn(&DBusServer) + 'static>(this: *mut ffi::GDBusServer, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(&from_glib_borrow(this))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"notify::client-address\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(notify_client_address_trampoline::<F> as *const ())), Box_::into_raw(f))
        }
    }
}
