...
impl Vfs {
    ...
    #[allow(clippy::should_implement_trait)]    pub fn default() -> Vfs {
        unsafe {
...
pub trait VfsExt: IsA<Vfs> + sealed::Sealed + 'static {
    ...
    fn register_uri_scheme(&self, scheme: &str, uri_func: Option<Box_<dyn Fn(&Vfs, &str) -> File + 'static>>, parse_name_func: Option<Box_<dyn Fn(&Vfs, &str) -> File + 'static>>) -> bool {
        let uri_func_data: Box_<Option<Box_<dyn Fn(&Vfs, &str) -> File + 'static>>> = Box_::new(uri_func);
        unsafe extern "C" fn uri_func_func(vfs: *mut ffi::GVfs, identifier: *const libc::c_char, user_data: glib::ffi::gpointer) -> *mut ffi::GFile {
            let vfs = from_glib_borrow(vfs);
            let identifier: Borrowed<glib::GString> = from_glib_borrow(identifier);
            let callback: &Option<Box_<dyn Fn(&Vfs, &str) -> File + 'static>> = &*(user_data as *mut _);
        ...
        let parse_name_func_data: Box_<Option<Box_<dyn Fn(&Vfs, &str) -> File + 'static>>> = Box_::new(parse_name_func);
        unsafe extern "C" fn parse_name_func_func(vfs: *mut ffi::GVfs, identifier: *const libc::c_char, user_data: glib::ffi::gpointer) -> *mut ffi::GFile {
            let vfs = from_glib_borrow(vfs);
            let identifier: Borrowed<glib::GString> = from_glib_borrow(identifier);
            let callback: &Option<Box_<dyn Fn(&Vfs, &str) -> File + 'static>> = &*(user_data as *mut _);
            if let Some(ref callback) = *callback {
        ...
        unsafe extern "C" fn uri_destroy_func(data: glib::ffi::gpointer) {
            let _callback: Box_<Option<Box_<dyn Fn(&Vfs, &str) -> File + 'static>>> = Box_::from_raw(data as *mut _);
        }
        ...
        let super_callback0: Box_<Option<Box_<dyn Fn(&Vfs, &str) -> File + 'static>>> = uri_func_data;
        let super_callback1: Box_<Option<Box_<dyn Fn(&Vfs, &str) -> File + 'static>>> = parse_name_func_data;
        unsafe {
            from_glib(ffi::g_vfs_register_uri_scheme(self.as_ref().to_glib_none().0, scheme.to_glib_none().0, uri_func, Box_::into_raw(super_callback0) as *mut _, destroy_call4, parse_name_func, Box_::into_raw(super_callback1) as *mut _, destroy_call7))
        }
    }
