...
pub trait SocketAddressEnumeratorExt: IsA<SocketAddressEnumerator> + sealed::Sealed + 'static {
    #[doc(alias = "g_socket_address_enumerator_next")]
    fn next(&self, cancellable: Option<&impl IsA<Cancellable>>) -> Result<Option<SocketAddress>, glib::Error> {
        unsafe {
    ...
    fn next_async<P: FnOnce(Result<Option<SocketAddress>, glib::Error>) + 'static>(&self, cancellable: Option<&impl IsA<Cancellable>>, callback: P) {
        ...
        unsafe extern "C" fn next_async_trampoline<P: FnOnce(Result<Option<SocketAddress>, glib::Error>) + 'static>(_source_object: *mut glib::gobject_ffi::GObject, res: *mut crate::ffi::GAsyncResult, user_data: glib::ffi::gpointer) {
            let mut error = ptr::null_mut();
        ...
        unsafe {
            ffi::g_socket_address_enumerator_next_async(self.as_ref().to_glib_none().0, cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box_::into_raw(user_data) as *mut _);
        }
    }

    
    fn next_future(&self) -> Pin<Box_<dyn std::future::Future<Output = Result<Option<SocketAddress>, glib::Error>> + 'static>> {
