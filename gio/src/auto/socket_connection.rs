...
impl SocketConnection {
    ...
    pub fn factory_lookup_type(family: SocketFamily, type_: SocketType, protocol_id: i32) -> glib::types::Type {
        unsafe {
            from_glib(ffi::g_socket_connection_factory_lookup_type(family.into_glib(), type_.into_glib(), protocol_id))
        }
    }
    ...
    pub fn factory_register_type(g_type: glib::types::Type, family: SocketFamily, type_: SocketType, protocol: i32) {
        unsafe {
            ffi::g_socket_connection_factory_register_type(g_type.into_glib(), family.into_glib(), type_.into_glib(), protocol);
        }
    }
}
...
pub trait SocketConnectionExt: IsA<SocketConnection> + sealed::Sealed + 'static {
    ...
    fn connect(&self, address: &impl IsA<SocketAddress>, cancellable: Option<&impl IsA<Cancellable>>) -> Result<(), glib::Error> {
        unsafe {
            ...
            let is_ok = ffi::g_socket_connection_connect(self.as_ref().to_glib_none().0, address.as_ref().to_glib_none().0, cancellable.map(|p| p.as_ref()).to_glib_none().0, &mut error);
    ...
    fn connect_async<P: FnOnce(Result<(), glib::Error>) + 'static>(&self, address: &impl IsA<SocketAddress>, cancellable: Option<&impl IsA<Cancellable>>, callback: P) {
        ...
        unsafe extern "C" fn connect_async_trampoline<P: FnOnce(Result<(), glib::Error>) + 'static>(_source_object: *mut glib::gobject_ffi::GObject, res: *mut crate::ffi::GAsyncResult, user_data: glib::ffi::gpointer) {
            let mut error = ptr::null_mut();
        ...
        unsafe {
            ffi::g_socket_connection_connect_async(self.as_ref().to_glib_none().0, address.as_ref().to_glib_none().0, cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box_::into_raw(user_data) as *mut _);
        }
    }

    
    fn connect_future(&self, address: &(impl IsA<SocketAddress> + Clone + 'static)) -> Pin<Box_<dyn std::future::Future<Output = Result<(), glib::Error>> + 'static>> {
