...
pub trait LoadableIconExt: IsA<LoadableIcon> + sealed::Sealed + 'static {
    ...
    fn load(&self, size: i32, cancellable: Option<&impl IsA<Cancellable>>) -> Result<(InputStream, glib::GString), glib::Error> {
        unsafe {
    ...
    fn load_async<P: FnOnce(Result<(InputStream, glib::GString), glib::Error>) + 'static>(&self, size: i32, cancellable: Option<&impl IsA<Cancellable>>, callback: P) {
        ...
        unsafe extern "C" fn load_async_trampoline<P: FnOnce(Result<(InputStream, glib::GString), glib::Error>) + 'static>(_source_object: *mut glib::gobject_ffi::GObject, res: *mut crate::ffi::GAsyncResult, user_data: glib::ffi::gpointer) {
            let mut error = ptr::null_mut();
            let mut type_ = ptr::null_mut();
        ...
        unsafe {
            ffi::g_loadable_icon_load_async(self.as_ref().to_glib_none().0, size, cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box_::into_raw(user_data) as *mut _);
        }
    }

    
    fn load_future(&self, size: i32) -> Pin<Box_<dyn std::future::Future<Output = Result<(InputStream, glib::GString), glib::Error>> + 'static>> {
