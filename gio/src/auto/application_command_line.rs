...
pub trait ApplicationCommandLineExt: IsA<ApplicationCommandLine> + sealed::Sealed + 'static {
    ...
    fn arguments(&self) -> Vec<std::ffi::OsString> {
        unsafe {
            ...
            let ret = FromGlibContainer::from_glib_full_num(ffi::g_application_command_line_get_arguments(self.as_ref().to_glib_none().0, argc.as_mut_ptr()), argc.assume_init() as _);
            ret
        }
    }
    ...
    fn options_dict(&self) -> glib::VariantDict {
        unsafe {
            from_glib_none(ffi::g_application_command_line_get_options_dict(self.as_ref().to_glib_none().0))
        }
    }
    ...
    fn connect_is_remote_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_is_remote_trampoline<P: IsA<ApplicationCommandLine>, F: Fn(&P) + 'static>(this: *mut ffi::GApplicationCommandLine, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
