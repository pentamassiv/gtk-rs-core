...
impl SimpleAction {
    ...
    #[doc(alias = "activate")]
    pub fn connect_activate<F: Fn(&Self, Option<&glib::Variant>) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn activate_trampoline<F: Fn(&SimpleAction, Option<&glib::Variant>) + 'static>(this: *mut ffi::GSimpleAction, parameter: *mut glib::ffi::GVariant, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(&from_glib_borrow(this), Option::<glib::Variant>::from_glib_borrow(parameter).as_ref().as_ref())
        }
        unsafe {
    ...
    pub fn connect_change_state<F: Fn(&Self, Option<&glib::Variant>) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn change_state_trampoline<F: Fn(&SimpleAction, Option<&glib::Variant>) + 'static>(this: *mut ffi::GSimpleAction, value: *mut glib::ffi::GVariant, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(&from_glib_borrow(this), Option::<glib::Variant>::from_glib_borrow(value).as_ref().as_ref())
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"change-state\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(change_state_trampoline::<F> as *const ())), Box_::into_raw(f))
        }
    }
}
