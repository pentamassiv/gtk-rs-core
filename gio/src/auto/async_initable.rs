...
pub trait AsyncInitableExt: IsA<AsyncInitable> + sealed::Sealed + 'static {
    ...
    unsafe fn init_async<P: FnOnce(Result<(), glib::Error>) + 'static>(&self, io_priority: glib::Priority, cancellable: Option<&impl IsA<Cancellable>>, callback: P) {
        ...
        unsafe extern "C" fn init_async_trampoline<P: FnOnce(Result<(), glib::Error>) + 'static>(_source_object: *mut glib::gobject_ffi::GObject, res: *mut crate::ffi::GAsyncResult, user_data: glib::ffi::gpointer) {
        ...
        ffi::g_async_initable_init_async(self.as_ref().to_glib_none().0, io_priority.into_glib(), cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box_::into_raw(user_data) as *mut _);
    }

    
    unsafe fn init_future(&self, io_priority: glib::Priority) -> Pin<Box_<dyn std::future::Future<Output = Result<(), glib::Error>> + 'static>> {
