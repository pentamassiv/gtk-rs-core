...
pub trait SimpleProxyResolverExt: IsA<SimpleProxyResolver> + sealed::Sealed + 'static {
    ...
    fn set_default_proxy(&self, default_proxy: Option<&str>) {
        unsafe {
            ffi::g_simple_proxy_resolver_set_default_proxy(self.as_ref().to_glib_none().0, default_proxy.to_glib_none().0);
        }
    }
    ...
    fn set_uri_proxy(&self, uri_scheme: &str, proxy: &str) {
        unsafe {
            ffi::g_simple_proxy_resolver_set_uri_proxy(self.as_ref().to_glib_none().0, uri_scheme.to_glib_none().0, proxy.to_glib_none().0);
        }
    }
    ...
    fn connect_default_proxy_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_default_proxy_trampoline<P: IsA<SimpleProxyResolver>, F: Fn(&P) + 'static>(this: *mut ffi::GSimpleProxyResolver, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(SimpleProxyResolver::from_glib_borrow(this).unsafe_cast_ref())
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"notify::default-proxy\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(notify_default_proxy_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }

    #[doc(alias = "ignore-hosts")]
    fn connect_ignore_hosts_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_ignore_hosts_trampoline<P: IsA<SimpleProxyResolver>, F: Fn(&P) + 'static>(this: *mut ffi::GSimpleProxyResolver, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(SimpleProxyResolver::from_glib_borrow(this).unsafe_cast_ref())
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"notify::ignore-hosts\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(notify_ignore_hosts_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
