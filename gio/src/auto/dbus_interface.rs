...
pub trait DBusInterfaceExt: IsA<DBusInterface> + sealed::Sealed + 'static {
    ...
    fn get(&self) -> Option<DBusObject> {
        unsafe {
            from_glib_full(ffi::g_dbus_interface_dup_object(self.as_ref().to_glib_none().0))
        }
    }
    ...
    fn info(&self) -> DBusInterfaceInfo {
        unsafe {
            from_glib_none(ffi::g_dbus_interface_get_info(self.as_ref().to_glib_none().0))
        }
    }
    ...
    fn set_object(&self, object: Option<&impl IsA<DBusObject>>) {
        unsafe {
            ffi::g_dbus_interface_set_object(self.as_ref().to_glib_none().0, object.map(|p| p.as_ref()).to_glib_none().0);
        }
    }
}
