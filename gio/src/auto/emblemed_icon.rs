...
impl EmblemedIcon {
    ...
    pub fn new(icon: &impl IsA<Icon>, emblem: Option<&Emblem>) -> EmblemedIcon {
        unsafe {
            from_glib_full(ffi::g_emblemed_icon_new(icon.as_ref().to_glib_none().0, emblem.to_glib_none().0))
        }
    }
}
...
pub trait EmblemedIconExt: IsA<EmblemedIcon> + sealed::Sealed + 'static {
    ...
    fn add_emblem(&self, emblem: &Emblem) {
        unsafe {
            ffi::g_emblemed_icon_add_emblem(self.as_ref().to_glib_none().0, emblem.to_glib_none().0);
        }
    }
