...
pub trait MenuModelExt: IsA<MenuModel> + sealed::Sealed + 'static {
    ...
    fn item_attribute_value(&self, item_index: i32, attribute: &str, expected_type: Option<&glib::VariantTy>) -> Option<glib::Variant> {
        unsafe {
...
#[must_use]
    ...
    fn connect_items_changed<F: Fn(&Self, i32, i32, i32) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn items_changed_trampoline<P: IsA<MenuModel>, F: Fn(&P, i32, i32, i32) + 'static>(this: *mut ffi::GMenuModel, position: libc::c_int, removed: libc::c_int, added: libc::c_int, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(MenuModel::from_glib_borrow(this).unsafe_cast_ref(), position, removed, added)
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"items-changed\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(items_changed_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
