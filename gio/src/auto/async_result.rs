...
pub trait AsyncResultExt: IsA<AsyncResult> + sealed::Sealed + 'static {
    ...
    fn source_object(&self) -> Option<glib::Object> {
        unsafe {
            from_glib_full(ffi::g_async_result_get_source_object(self.as_ref().to_glib_none().0))
        }
    }
    ...
    fn legacy_propagate_error(&self) -> Result<(), glib::Error> {
        unsafe {
            ...
            let is_ok = ffi::g_async_result_legacy_propagate_error(self.as_ref().to_glib_none().0, &mut error);
