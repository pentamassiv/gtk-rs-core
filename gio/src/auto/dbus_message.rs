...
use crate::{DBusCapabilityFlags,DBusMessageByteOrder,DBusMessageFlags,DBusMessageHeaderField,DBusMessageType};
#[cfg(unix)]
...
impl DBusMessage {
    ...
    pub fn from_blob(blob: &[u8], capabilities: DBusCapabilityFlags) -> Result<DBusMessage, glib::Error> {
        let blob_len = blob.len() as _;
        unsafe {
            let mut error = ptr::null_mut();
            let ret = ffi::g_dbus_message_new_from_blob(blob.to_glib_none().0, blob_len, capabilities.into_glib(), &mut error);
    ...
    pub fn new_method_call(name: Option<&str>, path: &str, interface_: Option<&str>, method: &str) -> DBusMessage {
        unsafe {
            from_glib_full(ffi::g_dbus_message_new_method_call(name.to_glib_none().0, path.to_glib_none().0, interface_.to_glib_none().0, method.to_glib_none().0))
        }
    }
    ...
    pub fn header(&self, header_field: DBusMessageHeaderField) -> Option<glib::Variant> {
        unsafe {
            from_glib_none(ffi::g_dbus_message_get_header(self.to_glib_none().0, header_field.into_glib()))
        }
    }
    ...
    pub fn unix_fd_list(&self) -> Option<UnixFDList> {
        unsafe {
            from_glib_none(ffi::g_dbus_message_get_unix_fd_list(self.to_glib_none().0))
        }
    }
...
#[must_use]
    ...
    pub fn set_header(&self, header_field: DBusMessageHeaderField, value: Option<&glib::Variant>) {
        unsafe {
            ffi::g_dbus_message_set_header(self.to_glib_none().0, header_field.into_glib(), value.to_glib_none().0);
        }
    }
    ...
    pub fn set_unix_fd_list(&self, fd_list: Option<&impl IsA<UnixFDList>>) {
        unsafe {
            ffi::g_dbus_message_set_unix_fd_list(self.to_glib_none().0, fd_list.map(|p| p.as_ref()).to_glib_none().0);
        }
    }
    ...
    pub fn to_blob(&self, capabilities: DBusCapabilityFlags) -> Result<Vec<u8>, glib::Error> {
        unsafe {
            ...
            let mut error = ptr::null_mut();
            let ret = ffi::g_dbus_message_to_blob(self.to_glib_none().0, out_size.as_mut_ptr(), capabilities.into_glib(), &mut error);
            if error.is_null() { Ok(FromGlibContainer::from_glib_full_num(ret, out_size.assume_init() as _)) } else { Err(from_glib_full(error)) }
        }
    }
    ...
    pub fn connect_locked_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn notify_locked_trampoline<F: Fn(&DBusMessage) + 'static>(this: *mut ffi::GDBusMessage, _param_spec: glib::ffi::gpointer, f: glib::ffi::gpointer) {
