...
use crate::{AsyncResult,BusType,Cancellable,DBusConnection,File,IOErrorEnum,IOStream,Icon,InputStream,Resource,ResourceLookupFlags,SettingsBackend};
use glib::{prelude::*,translate::*};
use std::{boxed::Box as Box_,mem,pin::Pin,ptr};


#[doc(alias = "g_bus_get")]
pub fn bus_get<P: FnOnce(Result<DBusConnection, glib::Error>) + 'static>(bus_type: BusType, cancellable: Option<&impl IsA<Cancellable>>, callback: P) {
    ...
    unsafe extern "C" fn bus_get_trampoline<P: FnOnce(Result<DBusConnection, glib::Error>) + 'static>(_source_object: *mut glib::gobject_ffi::GObject, res: *mut crate::ffi::GAsyncResult, user_data: glib::ffi::gpointer) {
    ...
    unsafe {
        ffi::g_bus_get(bus_type.into_glib(), cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box_::into_raw(user_data) as *mut _);
    }
}


pub fn bus_get_future(bus_type: BusType) -> Pin<Box_<dyn std::future::Future<Output = Result<DBusConnection, glib::Error>> + 'static>> {
...
pub fn bus_get_sync(bus_type: BusType, cancellable: Option<&impl IsA<Cancellable>>) -> Result<DBusConnection, glib::Error> {
    unsafe {
        let mut error = ptr::null_mut();
        let ret = ffi::g_bus_get_sync(bus_type.into_glib(), cancellable.map(|p| p.as_ref()).to_glib_none().0, &mut error);
...
pub fn content_type_get_mime_dirs() -> Vec<glib::GString> {
    unsafe {
        FromGlibPtrContainer::from_glib_none(ffi::g_content_type_get_mime_dirs())
    }
}
...
pub fn content_type_guess(filename: Option<impl AsRef<std::path::Path>>, data: &[u8]) -> (glib::GString, bool) {
    let data_size = data.len() as _;
    unsafe {
        let mut result_uncertain = mem::MaybeUninit::uninit();
        let ret = from_glib_full(ffi::g_content_type_guess(filename.as_ref().map(|p| p.as_ref()).to_glib_none().0, data.to_glib_none().0, data_size, result_uncertain.as_mut_ptr()));
...
pub fn dbus_address_get_for_bus_sync(bus_type: BusType, cancellable: Option<&impl IsA<Cancellable>>) -> Result<glib::GString, glib::Error> {
    unsafe {
        let mut error = ptr::null_mut();
        let ret = ffi::g_dbus_address_get_for_bus_sync(bus_type.into_glib(), cancellable.map(|p| p.as_ref()).to_glib_none().0, &mut error);
        if error.is_null() { Ok(from_glib_full(ret)) } else { Err(from_glib_full(error)) }
    }
}

#[doc(alias = "g_dbus_address_get_stream")]
pub fn dbus_address_get_stream<P: FnOnce(Result<(IOStream, Option<glib::GString>), glib::Error>) + 'static>(address: &str, cancellable: Option<&impl IsA<Cancellable>>, callback: P) {
    ...
    let user_data: Box_<glib::thread_guard::ThreadGuard<P>> = Box_::new(glib::thread_guard::ThreadGuard::new(callback));
    unsafe extern "C" fn dbus_address_get_stream_trampoline<P: FnOnce(Result<(IOStream, Option<glib::GString>), glib::Error>) + 'static>(_source_object: *mut glib::gobject_ffi::GObject, res: *mut crate::ffi::GAsyncResult, user_data: glib::ffi::gpointer) {
        let mut error = ptr::null_mut();
    ...
    unsafe {
        ffi::g_dbus_address_get_stream(address.to_glib_none().0, cancellable.map(|p| p.as_ref()).to_glib_none().0, Some(callback), Box_::into_raw(user_data) as *mut _);
    }
}


pub fn dbus_address_get_stream_future(address: &str) -> Pin<Box_<dyn std::future::Future<Output = Result<(IOStream, Option<glib::GString>), glib::Error>> + 'static>> {
...
pub fn dbus_address_get_stream_sync(address: &str, cancellable: Option<&impl IsA<Cancellable>>) -> Result<(IOStream, Option<glib::GString>), glib::Error> {
    unsafe {
        let mut out_guid = ptr::null_mut();
        let mut error = ptr::null_mut();
...
pub fn dbus_escape_object_path(s: &str) -> glib::GString {
    unsafe {
        from_glib_full(ffi::g_dbus_escape_object_path(s.to_glib_none().0))
    }
}
...
pub fn dbus_gvalue_to_gvariant(gvalue: &glib::Value, type_: &glib::VariantTy) -> glib::Variant {
    unsafe {
        from_glib_full(ffi::g_dbus_gvalue_to_gvariant(gvalue.to_glib_none().0, type_.to_glib_none().0))
    }
}
...
pub fn dbus_is_error_name(string: &str) -> bool {
    unsafe {
        from_glib(ffi::g_dbus_is_error_name(string.to_glib_none().0))
    }
}
...
pub fn keyfile_settings_backend_new(filename: &str, root_path: &str, root_group: Option<&str>) -> SettingsBackend {
    unsafe {
...
pub fn resources_enumerate_children(path: &str, lookup_flags: ResourceLookupFlags) -> Result<Vec<glib::GString>, glib::Error> {
    unsafe {
        let mut error = ptr::null_mut();
        let ret = ffi::g_resources_enumerate_children(path.to_glib_none().0, lookup_flags.into_glib(), &mut error);
        if error.is_null() { Ok(FromGlibPtrContainer::from_glib_full(ret)) } else { Err(from_glib_full(error)) }
    }
}

#[doc(alias = "g_resources_get_info")]
pub fn resources_get_info(path: &str, lookup_flags: ResourceLookupFlags) -> Result<(usize, u32), glib::Error> {
    unsafe {
        let mut size = mem::MaybeUninit::uninit();
        let mut flags = mem::MaybeUninit::uninit();
        let mut error = ptr::null_mut();
        let is_ok = ffi::g_resources_get_info(path.to_glib_none().0, lookup_flags.into_glib(), size.as_mut_ptr(), flags.as_mut_ptr(), &mut error);
        debug_assert_eq!(is_ok == glib::ffi::GFALSE, !error.is_null());
        if error.is_null() { Ok((size.assume_init(), flags.assume_init())) } else { Err(from_glib_full(error)) }
    }
}

#[doc(alias = "g_resources_lookup_data")]
pub fn resources_lookup_data(path: &str, lookup_flags: ResourceLookupFlags) -> Result<glib::Bytes, glib::Error> {
    unsafe {
        let mut error = ptr::null_mut();
        let ret = ffi::g_resources_lookup_data(path.to_glib_none().0, lookup_flags.into_glib(), &mut error);
        if error.is_null() { Ok(from_glib_full(ret)) } else { Err(from_glib_full(error)) }
    }
}

#[doc(alias = "g_resources_open_stream")]
pub fn resources_open_stream(path: &str, lookup_flags: ResourceLookupFlags) -> Result<InputStream, glib::Error> {
    unsafe {
        let mut error = ptr::null_mut();
        let ret = ffi::g_resources_open_stream(path.to_glib_none().0, lookup_flags.into_glib(), &mut error);
        if error.is_null() { Ok(from_glib_full(ret)) } else { Err(from_glib_full(error)) }
    }
}
