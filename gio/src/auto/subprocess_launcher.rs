...
impl SubprocessLauncher {
    ...
    pub fn new(flags: SubprocessFlags) -> SubprocessLauncher {
        unsafe {
            from_glib_full(ffi::g_subprocess_launcher_new(flags.into_glib()))
        }
    }
    ...
    pub fn set_child_setup<P: Fn() + 'static>(&self, child_setup: P) {
        ...
        unsafe {
            ffi::g_subprocess_launcher_set_child_setup(self.to_glib_none().0, child_setup, Box_::into_raw(super_callback0) as *mut _, destroy_call3);
        }
    }
    ...
    pub fn set_stderr_file_path(&self, path: Option<impl AsRef<std::path::Path>>) {
        unsafe {
            ffi::g_subprocess_launcher_set_stderr_file_path(self.to_glib_none().0, path.as_ref().map(|p| p.as_ref()).to_glib_none().0);
        }
    }
    ...
    pub fn set_stdout_file_path(&self, path: Option<impl AsRef<std::path::Path>>) {
        unsafe {
            ffi::g_subprocess_launcher_set_stdout_file_path(self.to_glib_none().0, path.as_ref().map(|p| p.as_ref()).to_glib_none().0);
        }
    }

    #[doc(alias = "g_subprocess_launcher_setenv")]
    pub fn setenv(&self, variable: impl AsRef<std::ffi::OsStr>, value: impl AsRef<std::ffi::OsStr>, overwrite: bool) {
        unsafe {
            ffi::g_subprocess_launcher_setenv(self.to_glib_none().0, variable.as_ref().to_glib_none().0, value.as_ref().to_glib_none().0, overwrite.into_glib());
        }
    }
