...
impl MemoryMonitor {
    ...
    pub fn dup_default() -> MemoryMonitor {
        unsafe {
            from_glib_full(ffi::g_memory_monitor_dup_default())
        }
    }
}
...
pub trait MemoryMonitorExt: IsA<MemoryMonitor> + sealed::Sealed + 'static {
    ...
    fn connect_low_memory_warning<F: Fn(&Self, MemoryMonitorWarningLevel) + 'static>(&self, f: F) -> SignalHandlerId {
        unsafe extern "C" fn low_memory_warning_trampoline<P: IsA<MemoryMonitor>, F: Fn(&P, MemoryMonitorWarningLevel) + 'static>(this: *mut ffi::GMemoryMonitor, level: ffi::GMemoryMonitorWarningLevel, f: glib::ffi::gpointer) {
            let f: &F = &*(f as *const F);
            f(MemoryMonitor::from_glib_borrow(this).unsafe_cast_ref(), from_glib(level))
        }
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, b"low-memory-warning\0".as_ptr() as *const _,
                Some(transmute::<_, unsafe extern "C" fn()>(low_memory_warning_trampoline::<Self, F> as *const ())), Box_::into_raw(f))
        }
    }
}
